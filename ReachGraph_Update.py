...
"""
def Merge_Clusters(Reachability_Graph_Mat, dest_clust_idx, src_clust_idx, dest_clust_reach_mat_idx, src_clust_reach_mat_idx):
	...
	for x in Cluster_Info_Dict[src_clust_idx]._GetPossibleR1List():
		Cluster_Info_Dict[dest_clust_idx]._AddPossibleR1(x)
		
	"""
	also we inspect whether the source cluster was itself part of some cluster's possible R1 list
	in such case, we replace the source cluster with the destination cluster information
	"""
	for clust_idx in Cluster_Info_Dict:
		if src_clust_idx in Cluster_Info_Dict[clust_idx]._GetPossibleR1List():
			Cluster_Info_Dict[clust_idx]._RemovePossibleR1(src_clust_idx)
			Cluster_Info_Dict[clust_idx]._AddPossibleR1(dest_clust_idx)
	
	"""
	finally, remove the src_clust_idx from the Candidate_Out_Edge_Cluster_List as well
	"""
	if src_clust_idx in Candidate_Out_Edge_Cluster_List:
		Candidate_Out_Edge_Cluster_List.remove(src_clust_idx)
...
def AdjustReachGraph(Reachability_Graph_Mat, nodeA_clust_idx, nodeB_clust_idx, reln_type):    
	...
	if (reln_type == RELATION_R3):      
		...
		Connect_ClusterPair(Reachability_Graph_Mat, nodeA_reach_mat_idx, nodeB_reach_mat_idx, RELATION_R1, nodeA_clust_idx, nodeB_clust_idx)
		...
		# connect the pair of clusters, along with updating the reachability matrix
		Connect_ClusterPair(Reachability_Graph_Mat, nodeB_reach_mat_idx, nodeA_reach_mat_idx, RELATION_R1, nodeB_clust_idx, nodeA_clust_idx)
		# now perform the transitive closure on the derived reachability matrix  
		TransClosUpd(Reachability_Graph_Mat, nodeA_reach_mat_idx, nodeB_reach_mat_idx, RELATION_R2)    
	else:	#reln_type == RELATION_R4:
		Connect_ClusterPair(Reachability_Graph_Mat, nodeA_reach_mat_idx, nodeB_reach_mat_idx, RELATION_R4, nodeA_clust_idx, nodeB_clust_idx)
		# now perform the transitive closure on the derived reachability matrix  
