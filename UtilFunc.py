...
5) type_of_output:
	if 0: average of measure is returned
	if 1: max of measures is returned
	if 2: sum of measures is returned

	Otherwise 0 is returned if the cluster pair is not supported at all
...
def FindAvgDistanceMeasure(taxa_clust1, taxa_clust2, DIST_MAT_TYPE, single_elem=2, type_of_output=0):
		...
		for x2 in taxa_list2:  
			...
			if target_key in TaxaPair_Reln_Dict:
				if (DIST_MAT_TYPE == 1) or (DIST_MAT_TYPE == 2):
					...
					val = TaxaPair_Reln_Dict[target_key]._GetEdgeCost_ConnReln(RELATION_R1)
				elif (DIST_MAT_TYPE == 4):
					val = TaxaPair_Reln_Dict[target_key]._GetEdgeWeight(RELATION_R1)
				elif (DIST_MAT_TYPE == 5):
					val = TaxaPair_Reln_Dict[target_key]._GetConnPrVal(RELATION_R1)
				"""
	...
	if (len(curr_taxa_pair_list) > 0):
		if (type_of_output == 0):
			...
			return sum(curr_taxa_pair_list)
	else:
		return 0
