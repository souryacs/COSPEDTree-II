	...
	#for i in range(no_of_clusters - 1):
		#for j in range(i+1, no_of_clusters):
				...
				#for reln_type in RELN_LIST:	
						...
						#Connect_ClusterPair(Reachability_Graph_Mat, CURRENT_CLUST_IDX_LIST.index(clust1), \
							...
							#for x in y_possible_R2_list:
										...
										#(Reachability_Graph_Mat[CURRENT_CLUST_IDX_LIST.index(cx)][CURRENT_CLUST_IDX_LIST.index(x)] == 0):
											...
											#fp.write('\n We have ' + str(cx) + '-->' + str(y) + '  and  ' + str(x) + '--->' + str(y) + \
												#'  ===>>> So inserting ' + str(x) + '----> ' + str(cx) + '  since they are not related ')
										## establish x--->cx
										#Connect_PossibleR1Reln_ClusterPair(Reachability_Graph_Mat, x, cx)
										#flag = True
	...
	for cx in Cluster_Info_Dict:
		...
		if (len(cx_R2_list) > 1):
			...
			for cz in cx_R2_list:
				if (measure == 1) or (measure == 2):
					cz_score = FindAvgDistanceMeasure(Cluster_Info_Dict[cz]._GetSpeciesList(), Cluster_Info_Dict[cx]._GetSpeciesList(), measure, 2, 1)
				else:
					cz_score = FindAvgDistanceMeasure(Cluster_Info_Dict[cz]._GetSpeciesList(), Cluster_Info_Dict[cx]._GetSpeciesList(), measure, 2, 2)	#0
			...
			if (DEBUG_LEVEL >= 2):
				fp.write('\n Initially selected target_score_clust_idx: ' + str(target_score_clust_idx) + '  target_score_clust: ' + str(target_score_clust))
